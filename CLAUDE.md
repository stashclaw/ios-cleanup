...
| `HomeViewModel` | `Views/HomeViewModel.swift` | `@MainActor ObservableObject`. All scan functions. `ScanState` enum (conforms to `Equatable`). UUID-guard stale-write prevention. Persistent `totalCleanedBytes` via `@AppStorage`. Scan result caching via `UserDefaults`. `PHFetchOptions` uses `includeAllBursts: true` for accurate photo counts. **Category card navigation**: if results cached → navigate immediately; if not → trigger scan + auto-navigate on completion. **`reviewedAssetIDs: Set<String>`** — flat set of asset localIdentifiers belonging to reviewed (completed, skipped, or queued) groups. Built by `refreshReviewedGroupKeys()` which reads UserDefaults completed/skipped group keys, maps them to asset IDs via `photoGroups`, and unions with markedIDs. Called on `init`, `didFreeBytes`, `onAppear`, and whenever `reviewVM` count changes in PhotoResultsView. HomeView uses this for O(1) chip-count filtering (avoids sort+join per render that would freeze during scans). |
...
| `PhotoResultsView` | `Views/Photos/PhotoResultsView.swift` | 2-col `LazyVGrid`, filter pills (dynamic), bulk select (paid). Takes `title: String` + pre-filtered `groups`. "Review All" button launches `GroupReviewView`. Floating delete bar shows queued count; "Delete All" is paid gate. Passes `$reviewVM.markedIDs` binding to `PhotoGroupDetailView`. **Group lifecycle**: once a group is skipped or queued/deleted, it is REMOVED from the main grid permanently and never shown again. Skipped groups move to a "SKIPPED" section at the bottom with a Restore button. The main grid count (hero card) reflects only remaining unreviewed groups. Three-condition filter: `skippedGroupKeys` (from reviewVM, @Published), `completedGroupKeys` (from reviewVM, @Published), `markedIDs` (from reviewVM). Home chip counts stay in sync via `.onChange` on reviewVM counts → `homeViewModel.refreshReviewedGroupKeys()`. |
...
| `GroupReviewViewModel` | `Views/Photos/GroupReviewViewModel.swift` | `@MainActor ObservableObject`. `queue: [PhotoGroup]`, `currentIndex`, `markedIDs` (persisted to UserDefaults via `didSet`). `bestIDForCurrentGroup`, `pendingMarked` per-group state. `selectBest(_:)` → re-auto-marks others. `togglePendingMark(_:)` → cannot unmark the best. `queueAndAdvance()` commits pending + steps AND calls `persistCompleted()`. `skipGroup()` steps without queuing AND calls `persistSkip()`. `commitDeletes()` → marks all queued groups as completed BEFORE clearCache() so groups stay hidden after markedIDs is cleared. `hasCachedSession` for crash recovery. `startFresh()` clears cache. `buildDecisions()` extracts `VNFeaturePrintObservation` vectors + quality scores before deletion and records `UserDecision` entries in `UserDecisionStore.shared`. **Two @Published group-state sets**: `completedGroupKeys` (queued+deleted, survives clearCache) and `skippedGroupKeys` (skipped without queueing, shows in Skipped section). Both are persisted to UserDefaults and reactive — PhotoResultsView subscribes directly. `unskipGroup(_:)` instance method removes from skippedGroupKeys. |
