...
| `PhotoMLStore` | SQLite-backed (`libsqlite3`) feature store for ML training data. Tables: `photo_features`, `photo_asset_analysis` (warm-scan cache; joins `photo_features` for embeddings rather than duplicating them), `pairwise_similarity`, `feedback_events`, `training_rows`. Schema version is recorded in `schema_meta` and gates migrations; opening a file from a newer build is refused rather than silently downgraded. CSV export for CreateML. Located at `Application Support/PhotoDuck/ml/photoduck-ml.sqlite`. |
...
- **`HomeViewModel`** (`@MainActor ObservableObject`) — owns the photo and file scan engines and the cleanup dashboard. Persists scan state to `UserDefaults` under key `photoduck.cleanup-state.v2` as JSON. Tracks freshness with `CleanupResultsFreshnessState` (`.live` / `.lastKnown` / `.stale`).
...
├── OnboardingView          — permissions (Photos)
...
**Paid:** Auto-clean all groups, custom multi-select photo deletion, and video compression. **Free:** classifier-selected Keep Best for one eligible group, individual file deletion, and user-authored Duck Mode swipe commits. Never collect review effort and then paywall its commit. `visuallySimilar` groups remain review-only and never expose automatic deletion.
